  ...
  <div class="col-md-9">
    <div class="row product-list">
        ...
        <tbody>
            ...
            <?php $i = 1; ?>
            @foreach ($catalogues as $list)
            <tr>
              <td>{{$i}}</td>
              <td>{{$list->catalogue_name}}</td>
              ...
              <td>
                <a href="{{ url('addCatalogue',$list->id)}}" class="btn btn-sm btn-warning"><i class="fa fa-edit"></i></a>
...
<script>
    $('.toggle-class').on("change", function() {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "<?php echo url('/catalogueStatus'); ?>",
            data: {
                _token: '{{ csrf_token() }}',
                'status': status,
                'id': id,
